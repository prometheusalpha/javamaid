...
def parse(java_codes):
    ...
    for java_module in java_codes:
            mermaid_diagram += convert_to_mermaid(java_module.replace("\"\"\"", "\"").replace("\n", ""))
...
def convert_to_mermaid(java_code):
    ...
    return convert_to_mermaid_based_on_type(type, java_code)

def get_type(java_code):
    tree = javalang.parse.parse(java_code)
    return tree.types[0].__class__.__name__

def convert_to_mermaid_based_on_type(type, java_code):
